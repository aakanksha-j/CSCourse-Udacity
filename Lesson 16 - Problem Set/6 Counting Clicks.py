...
def check_word_repetition(url,urls):
    for element in urls:
        if element[0] == url:
            return False
    return True
...
def add_to_index(index, keyword, url):
    for entry in index:
        if entry[0] == keyword:
            if check_word_repetition(url,entry[1]):
...
quite good at  <a href="http://www.udacity.com/cs101x/kicking.html">kicking good good good</a>.
</body> </html>'''
