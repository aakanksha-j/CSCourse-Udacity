...
def ordered_search(index, ranks, keyword):
    ...
    for url in index[keyword]:
        url_rank_list.append(ranks[url])
        slice_of_rank[url] = ranks[url]
    #print(url_rank_list)

    def quick_sort(l, r): # descending order
        def partition(l, r):

            pivot = random.randint(l, r)
            url_rank_list[pivot], url_rank_list[r] = url_rank_list[r], url_rank_list[pivot]

            i = l - 1
            for j in range(l, r):
                if url_rank_list[j] > url_rank_list[r]:
                    i += 1
                    url_rank_list[i], url_rank_list[j] = url_rank_list[j], url_rank_list[i]
            url_rank_list[i + 1], url_rank_list[r] = url_rank_list[r], url_rank_list[i+1]
    ...
    quick_sort(0, len(url_rank_list) - 1)

    output = []
    #print(url_rank_list)
    for url_rank in url_rank_list:
        for key, value in slice_of_rank.items():
            if url_rank == value:
                output.append(key)
